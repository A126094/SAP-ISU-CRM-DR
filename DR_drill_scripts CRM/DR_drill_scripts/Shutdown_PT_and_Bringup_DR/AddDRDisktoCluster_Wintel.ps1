...
foreach($disk1 in $disks)
{
 ...
 if($temp.OwnerNode -match $server1)
 {
  ...
  if(($temp.DriveLetter -notmatch "CP1DATA") -and ($temp.DriveLetter -ne "Not found") -and ($temp.VolumeName -ne "CP1LOG"))
  {
  ...
  Else
  {
   if(($temp.DriveLetter -notmatch "CP1DATA1") -and ($temp.DriveLetter -eq "Not found") -and ($temp.VolumeName -ne "CP1LOG"))
   {
   $accesspath = "G:\"+$temp.VolumeName
   Write-Host "Access path $accesspath" -ForegroundColor Green
   Add-PartitionAccessPath -DiskNumber $temp.PhysicalDiskNumber -PartitionNumber 2 -AccessPath $accesspath
 ...
 Else
 {
   ...
   if(($temp.DriveLetter -notmatch "CP1DATA1") -and ($temp.DriveLetter -ne "Not found") -and ($temp.DriveLetter -ne "CP1LOG"))
  {
   $accesspath = "G:\"+$temp.VolumeName
  ...
  Else
  {
   if(($temp.DriveLetter -notmatch "CP1DATA1") -and ($temp.DriveLetter -eq "Not found") -and ($temp.DriveLetter -ne "CP1LOG"))
   {
   $accesspath = "G:\"+$temp.VolumeName
   Add-PartitionAccessPath -CimSession $session1 -DiskNumber $temp.PhysicalDiskNumber -PartitionNumber 2 -AccessPath $accesspath
   }
