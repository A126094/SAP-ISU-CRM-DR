...
Write-host "*******************************************Removal of PT disks from the cluster has been started***********************************" -ForegroundColor Green
...
foreach($lun in $data)
{
 if(($lun.VolumeName -match "CP1DATA") -or ($lun.VolumeName -eq "CP1LOG"))
 {
  $disks += $lun.ClusterDiskNumber
 }
}
$disks
$RemovedLuns = @()
...
$cdisk
 ...
 $lunid = $disk1.ScsiLun
 Get-ClusterResource "SQL Server (SQLSVRCPT)" | Remove-ClusterResourceDependency -Resource $cdisk
