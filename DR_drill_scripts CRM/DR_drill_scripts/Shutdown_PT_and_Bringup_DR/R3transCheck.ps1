...
"*******************************************************************" | Add-Content $OutputFilePath
	...
	$Status=(& "\\sapcrmdclst01\sapmnt\CP1\SYS\exe\uc\NTAMD64\R3trans" -d | Out-String).Trim();
...
"*******************************************************************" | Add-Content $OutputFilePath
$command = @'
cmd.exe /C "sqlcmd -S $Servername -d $Database -i $SQLScriptPath\OrphanProblem_CRM.sql -o $OutputPath\Orphan_Login_CRM.txt"
'@
