...
"Removal of DR disks from cluster started: $((Get-Date).ToString())" | Add-Content $OutputFilePath
"*******************************************************************" | Add-Content $OutputFilePath

Write-host "*******************************************Removal of DR disks from the cluster has been started***********************************" -ForegroundColor Green

if((Test-Path "E:\DR_drill_scripts\Shutdown_PT_and_Bringup_DR\TempDRDisks.txt") -and ((Get-Content "E:\DR_drill_scripts\Shutdown_PT_and_Bringup_DR\TempDRDisks.txt") -ne $null) )
{
  $disks = Get-Content "E:\DR_drill_scripts\Shutdown_PT_and_Bringup_DR\TempDRDisks.txt"
  
  foreach($cdisk in $disks)
 {
   $cdisk
   $DiskResource1 = gwmi  MSCluster_Resource -Namespace root/mscluster | ?{ $_.Name -eq $cdisk }
   $Disk1 = gwmi -Namespace root/mscluster -Query “Associators of {$DiskResource1} Where ResultClass=MSCluster_Disk”
   $lunid = $disk1.ScsiLun
   Get-ClusterResource "SQL Server" | Remove-ClusterResourceDependency -Resource $cdisk
   Get-ClusterResource $cdisk | Suspend-ClusterResource -ErrorAction SilentlyContinue
   Get-ClusterResource $cdisk | Stop-ClusterResource -IgnoreLocked -ErrorAction SilentlyContinue
   Get-ClusterResource $cdisk | Move-ClusterResource -Group "Available Storage" -ErrorAction SilentlyContinue
   Get-ClusterResource $cdisk | Remove-ClusterResource -Force
   $RemovedLuns +=$lunid
...
foreach($ClusterDisk in $ClusterDisks)
{
 ...
 $temp |Export-csv E:\DR_drill_scripts\Shutdown_PT_and_Bringup_DR\TempDR.csv -NoTypeInformation -Append
...
$data = Import-csv E:\DR_drill_scripts\Shutdown_PT_and_Bringup_DR\TempDR.csv
$data | FT
$data.Count
...
Import-csv E:\DR_drill_scripts\Shutdown_PT_and_Bringup_DR\TempDR.csv | ConvertTo-Html -head $head -body $bodyformat | Out-File E:\DR_drill_scripts\Shutdown_PT_and_Bringup_DR\TempDR.html
...
'@
    ...
    $WebBrowser.Navigate("E:\DR_drill_scripts\Shutdown_PT_and_Bringup_DR\TempDR.html")
...
$cdisk
 ...
 $lunid = $disk1.ScsiLun
 Get-ClusterResource "SQL Server" | Remove-ClusterResourceDependency -Resource $cdisk
 Get-ClusterResource -Cluster $ClusterName $cdisk | Suspend-ClusterResource -ErrorAction SilentlyContinue
 Get-ClusterResource -Cluster $ClusterName $cdisk | Stop-ClusterResource -IgnoreLocked -ErrorAction SilentlyContinue
...
"Removal of DR disks from cluster finished: $((Get-Date).ToString())" | Add-Content $OutputFilePath
"*******************************************************************" | Add-Content $OutputFilePath

$var = new-object -comobject wscript.shell;
$intAnswer = $var.popup("Do you want to proceed Storage PT map task?",0,"DR removal Wintel",48+4)
If ($intAnswer -ne 6) 
{
