...
. E:\DR_drill_scripts\Shutdown_PT_and_Bringup_DR\AttachDettach_DB_ISU.ps1
...
. E:\DR_drill_scripts\Shutdown_PT_and_Bringup_DR\Configuration_modify_ISU.ps1
...
do
{
	...
	$args = "E:\DR_drill_scripts\Shutdown_PT_and_Bringup_DR\Shutdown_PT_ISU.ps1"
...
switch ( $Choice)
{
	0
	{
		...
		attachDatabaseISU; #This willl attach Production DR copy IP1
		OrphanLoginProblem;
		startClusterGroup("SAP IP1"); ## Start the Cluster Group
		checkClusterStatus("SAP IP1"); ##Failover test
		checkClusterStatus("SAP IP1"); ##Failback to original cluster test
		CheckR3trans;
		clearTableContentISU; ## Clear table and SQL queries for ISU
		StartSAPSystem; ## To start the SAP DR system IP1
		startSICK; ## Run SICK T-Code in DR system
	}
	1
	{
		$startProc1=Start-Process powershell.exe -Verb runas -ArgumentList ("-file $args1") -PassThru
