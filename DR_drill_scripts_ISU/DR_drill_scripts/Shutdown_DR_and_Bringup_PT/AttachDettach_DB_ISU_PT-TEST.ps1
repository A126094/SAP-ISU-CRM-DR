...
Invoke-sqlcmd -inputfile "$ScriptPath\Detach_DR_DB_ISU.sql" -serverinstance "$ServernameDR" -ConnectionTimeout 360
...
push-location
import-module sqlps -disablenamechecking
invoke-sqlcmd -inputfile "$ScriptPath\Attach_PT_DB_ISU.sql" -serverinstance "$ServernamePT" -ConnectionTimeout 180
$dbs = $server.Databases;
$DBStatus="Not Exist";
	foreach ($db in $dbs) 
	{
